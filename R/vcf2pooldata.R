...
#' @param poolsizes A numeric vector with haploid pool sizes (if NA haploid pools sizes are set to a default value of 100)
#' @param poolnames A character vector with the names of pool (if NA sample names are retrieved from the vcf header)
...
vcf2pooldata<-function(vcf.file="",poolsizes=NA,poolnames=NA,min.cov.per.pool=-1,min.rc=1,max.cov.per.pool=1e6,min.maf=-1,remove.indels=FALSE,min.dist.from.indels=0,nlines.per.readblock=1000000,verbose=TRUE){
  ...
  #retrieve (or check dimension if provied) of sample names
  if(sum(is.na(poolnames))>0){
    poolnames=tmp.data[-(1:9)]
    if(verbose){
      cat("  *The following sample names for the",npools,"pools were retrieved from the vcf:\n")
      print(poolnames)
    }
  }else{
  ...
  #give (or check dimension if provied) of haploid sample sizes
  if(sum(is.na(poolsizes))>0){#set to a default value of 100
    poolsizes=rep(100,npools) 
    if(verbose){
    cat("  *Warning: No pool sizes available from poolsizes argument\n  => the (haploid) pool size were set to an arbitrary default value of 100 for the",npools,"pools included in the vcf.\n")
    cat("  !!!!Don't forget to update the slot @poolsizes of the output pooldata object if needed!!!!\n")
    }
  }else{#check if provided
