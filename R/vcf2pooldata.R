...
#' @description Convert VCF files into a pooldata object. Only bi-allelic SNP are here considered.
...
while(continue.reading){
 ...
 if(substr(tmp.data[1],3,8)=="FORMAT"){
   n.index=n.index+1
   if(grepl("ID=RD,",tmp.data[1])){rd.index=n.index}
   if(grepl("ID=AD,",tmp.data[1])){ad.index=n.index}
 }
...
# tmp.data=matrix(scan(file=sync.file,skip=skip,nlines = nlines.per.readblock,what="character",quiet=TRUE),ncol=npools+3,byrow=T)  
 ...
 tmp.data=tmp.data[!grepl(",",tmp.data[,5]),]
 npos=nrow(tmp.data)
 if(npos>1){
  tmp.snpdet=tmp.data[,c(1,2,4,5)]
  tmp.cnt=tmp.data[,-1:-9]
  rm(tmp.data) 
  tmp.Y=tmp.N=matrix(0,npos,npools)
  for(i in 1:npools){
   tmp.snpcalled=lengths(regmatches(tmp.cnt[,i], gregexpr(":", tmp.cnt[,i]))) == (n.index - 1)
   tmp.cnt2=matrix((unlist(strsplit(tmp.cnt[tmp.snpcalled,i],split = ":"))),ncol=n.index,byrow=TRUE)[,c(rd.index,ad.index)]
   tmp.Y[tmp.snpcalled,i]=as.numeric(tmp.cnt2[,1])
   tmp.N[tmp.snpcalled,i]=as.numeric(tmp.cnt2[,2]) + tmp.Y[tmp.snpcalled,i]   
   }
...
rm(data.N)
res@snp.info=snpdet[,c(1:2,3,3:4)] #here the ref allele is always the allele in the reference assembly
rm(snpdet)
